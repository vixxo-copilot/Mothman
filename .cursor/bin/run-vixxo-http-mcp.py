#!/usr/bin/env python3
"""Stdio bridge for Vixxo remote HTTP MCP servers (via mcp-remote + Bearer auth)."""
...
SERVERS: dict[str, tuple[str, tuple[str, ...], tuple[str, ...]]] = {
    ...
    "vixxolink": (
        ...
        ("VIXXOLINK_API_TOKEN", "GATEWAY_API_TOKEN", "VIXXONOW_API_TOKEN"),
        ("vixxolink_api_token", "vixxolink_token", "vixxonow_api_token", "gateway_api_token"),
    ),
    "vixxonow": (
        "https://vixxonow.com/mcp/vixxonow",
        ("VIXXONOW_API_TOKEN", "VIXXOLINK_API_TOKEN", "GATEWAY_API_TOKEN"),
        ("vixxonow_api_token", "vixxonow_token", "vixxolink_api_token", "gateway_api_token"),
    ),
    "business-objects": (
    ...
    "powerbi-prod": (
        ...
        ("VIXXONOW_API_TOKEN", "POWERBI_API_TOKEN", "GATEWAY_API_TOKEN"),
        ("vixxonow_api_token", "powerbi_api_token", "gateway_api_token"),
    ),
    "gong-transact": (
    ...
    "chatfpt": (
        ...
        ("CHATFPT_API_TOKEN", "VIXXONOW_API_TOKEN", "GATEWAY_API_TOKEN"),
        ("chatfpt_api_token", "vixxonow_api_token", "gateway_api_token"),
    ),
    "dynamics365": (
        "https://vixxonow.com/mcp/dynamics365",
        ("VIXXONOW_API_TOKEN", "DYNAMICS_CLIENT_SECRET", "GATEWAY_API_TOKEN"),
        ("vixxonow_api_token", "dynamics_client_secret", "gateway_api_token"),
    ),
}
...
def main() -> int:
    ...
    if not token:
        print(
            f"{server_key} MCP: API token not set — refusing to start.",
            file=sys.stderr,
        )
        print(
