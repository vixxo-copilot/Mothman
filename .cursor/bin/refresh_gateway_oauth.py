#!/usr/bin/env python3
"""One-time Gateway authorization-code login. Opens Chrome; no token values printed."""
...
AUTHORIZE_URL = "https://vixxonow.com/mcp/gateway/oauth/authorize"
REDIRECT = "http://127.0.0.1:29069/oauth/callback"
CHROME = Path(r"C:\Program Files\Google\Chrome\Application\chrome.exe")
CLIENT_INFO = Path.home() / ".mcp-auth" / "mcp-remote-v1" / f"{GATEWAY_AUTH_ID}_client_info.json"
TOKENS = Path.home() / ".mcp-auth" / "mcp-remote-v1" / f"{GATEWAY_AUTH_ID}_tokens.json"
SYNC = Path(__file__).resolve().parent / "sync_gateway_token.py"
...
class _Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        ...
        if qs.get("error"):
            ...
            body = b"Gateway sign-in complete. You can close this tab."
...
def main() -> int:
    ...
    server = HTTPServer(("127.0.0.1", 29069), _Handler)
    thread = threading.Thread(target=server.serve_forever, daemon=True)
    thread.start()
    print("callback=http://127.0.0.1:29069/oauth/callback")
    print("Opening Chrome for Gateway sign-in...")
    if CHROME.is_file():
    ...
    print(f"tools_ok={mcp_tools_list_ok(GATEWAY_URL, access.strip())}")
