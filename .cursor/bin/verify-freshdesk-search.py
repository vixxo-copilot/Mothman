#!/usr/bin/env python3
"""Verify patched Freshdesk search query formatting against live API."""
...
def format_freshdesk_search_query(query: str) -> str:
    trimmed = query.strip()
...
def search(domain: str, api_key: str, query: str, page: int = 1) -> tuple[int, object]:
    params = {"query": format_freshdesk_search_query(query), "page": str(page)}
...
def main() -> int:
    ...
    for query in cases:
        ...
        print(f"patched query={query!r}: HTTP {status} results={count}")
        if status != 200:
            failed += 1
