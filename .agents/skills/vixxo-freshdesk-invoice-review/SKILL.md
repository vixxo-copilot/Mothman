...
|---|---|---|
| Payment follow-up / unpaid invoice / statement (SPM stay) | Body/subject intent asks about **unpaid / past-due / overdue / outstanding** invoices, payment timing ("when will we be paid"), aging, `statement of account`, multi-invoice AR list, or balance chase — **even when an attachment (xlsx/pdf statement) is present**. Classify from **full email body + conversation context**, not subject or attachment filename alone. | **B14-unpaid-invoice-spm-follow-up** (added v1.18). **Do not** treat as bucket-4 invoice intake, **do not** set `cf_type_of_request` to `Submit an Invoice for manual entry`, **do not** `/forward` to `invoices@vixxo.com`, **do not** move group off SPM, **do not** close. Identify the SP (see B14 SP-identity step), set `cf_type_of_request: Follow up on an Unpaid Invoice`, keep `group_id:159000485013` (SPM), leave **Open**, tag for SPM unpaid follow-up. Narrow exception: a **single-SR** ask where Gateway already returns a clean paid/pending/rejected answer with no multi-invoice aging list may still use Bucket 3 payment-status reply — only after body review confirms it is not an SPM unpaid follow-up. Audit: ticket **#56213** (Facility Plus statement / past due) was wrongly closed with `invoices-forward` + manual-entry type. |)
...
3. **For each ticket on the page:**
   ...
   f. If the bucket is 2, 3, 4, B10-manual-entry-vint, B11-incorrect-invoice-totals-vint, B12-supplemental-invoice-needed, B13-sr-requiring-updates-corp-ap, or B14-unpaid-invoice-spm-follow-up: extract the SR number (and for B14, company/contact identifiers) from ticket content, then pull Gateway context in order — service request → invoice + line items → quote + line items. For **B14**, also run SP identity via `sp-inbound-vetting` and/or VixxoLink / VixxoNow when Gateway alone does not resolve the SP. **Skip Gateway entirely for buckets 0, 1-close, 1-follow-up, 5, 6, 7, and `manual-review-recipient-exclusion`** (Gateway data doesn't inform those decisions; pulling it would waste API budget and, for bucket 0, could lend credibility to a fake SR # fabricated by an attacker).
...
4. **Check routing overlays (in order — first match wins):**
   - **B14-unpaid-invoice-spm-follow-up** if the **email body** (not just subject/attachment) asks about unpaid / past-due / outstanding invoices, payment timing, aging, or a statement-of-account chase. Prefer body intent over attachment presence. Do **not** confuse with B13 (Corp AP `SR Requiring Updates` / deposit verification) or B10 (explicit manual-entry / Delinquent SC Invoice intake).
...
5. **Extract the SR number and identify the SP.** Look in subject, body, and conversation replies. Providers are inconsistent about where they put it. If multiple SR numbers appear, prefer the one in the subject or the one referenced most frequently in the thread — and flag the others. For **B14** (and whenever SP identity is unclear on unpaid/statement tickets), resolve the service provider via **`sp-inbound-vetting`** (Skills MCP `get_skill("sp-inbound-vetting")`) and/or cross-reference **Gateway**, **VixxoLink**, and **VixxoNow** using company name, requester email, SR#, invoice#, or account codes in the body. Record the SP number/name for `cf_sp` before any ticket field update.
...
**Does not change:** B10 (manual entry / delinquent → VINT), B11 (incorrect totals → VINT, keep open), B12 (supplemental balance), B13 (Corp AP), B14 (unpaid/statement stay SPM), phishing, onboarding, SAP, or vendor-hold pre-screens still win when they fire.
...
6. **SPM invoice-concern category overlays** — unpaid/statement SPM follow-ups (**B14**), VINT routes (manual entry, delinquent, incorrect totals), supplemental-invoice-after-accepted cases, SR Requiring Updates / Corp AP deposit verification, manual invoice intake, Gateway/AP exceptions, VixxoLink blockers, duplicates, payment-link/remit risk, missing identifiers, and credit-card/internal-process tickets are mapped to the safest bucket before the generic keyword rules. These overlays prevent incorrect totals from being auto-closed (#45514), delinquent tickets from staying in SPM (#46617), accepted invoices with omitted balances from bucket-4 close (#47896), Corp AP tickets from missing `cf_type_of_request` (#48269), and unpaid/statement chases from being closed as invoice intake (#56213 → B14 stay SPM).
...
| 3 | Payment follow-up / aging (narrow — single-SR Gateway answer only) | Single-SR payment-status ask where body review confirms a clean Gateway-backed paid/pending/rejected answer is appropriate. | Gateway research. Draft customer reply with payment status (paid / pending / on hold / rejected) and any reason codes. **Do not use Bucket 3** when the body is an unpaid / past-due / statement-of-account / multi-invoice AR chase — that is **B14-unpaid-invoice-spm-follow-up** (stay SPM, set `Follow up on an Unpaid Invoice`, identify SP, leave Open). **Exception:** if the ticket is an SP/provider asking for an update or asking why an SR/invoice/NTE approval has not moved forward and there are no attachments to process, classify as **1-follow-up** instead. That work needs AP Team ownership, an open ticket, and routing tags; closing with a note does not move it forward. |
...
| 3 | Customer-facing reply with payment status and any reason codes — **only** for the narrow single-SR Gateway-answer case. If body intent is unpaid/statement/multi-invoice AR chase → use **B14**, not this row. | Relationship-first, definitive where possible |
...
| B14-unpaid-invoice-spm-follow-up | Identify SP via `sp-inbound-vetting` and/or Gateway / VixxoLink / VixxoNow. Internal note summarizing unpaid ask + SP identity + key SRs/invoices from the body. Ticket update: `type: Invoice Support`, `cf_type_of_request: Follow up on an Unpaid Invoice`, `cf_sp` populated, **`group_id` stays SPM `159000485013`**, tags `unpaid-invoice-follow-up` + `spm-unpaid-follow-up`, **Open (`status=2`)**. **No** `/forward` to `invoices@vixxo.com`, **no** VINT/Corp AP group move, **no** close, **no** payment promise. Optional brief receipt reply only after operator approval. | Optional brief receipt — no payment date promise |
...
## Bucket B14-unpaid-invoice-spm-follow-up — Unpaid / statement chase (stay SPM, Open)
| Ticket | Subject | Requester | Source tags | Ambiguity | Findings | Recommended action |
|---|---|---|---|---|---|---|
| #### | ... | ... | qsiap-source / — | — | Body asks unpaid/past-due/statement; SP identified via vetting/Gateway/VixxoLink | Identify SP; set `cf_type_of_request: Follow up on an Unpaid Invoice` + `cf_sp`; **stay SPM**; tags `unpaid-invoice-follow-up` + `spm-unpaid-follow-up`; leave Open (audit: #56213) |
...
- Bucket B14-unpaid-invoice-spm-follow-up (SPM unpaid follow-up, Open): N
...
- Change ticket group ownership with `tickets_manage update` when routing requires it: B10-manual-entry-vint (`group_id:159000486559` on close), B11-incorrect-invoice-totals-vint (`group_id:159000486559`, stay Open/Pending), B13-sr-requiring-updates-corp-ap (`group_id:159000486566`, Pending + `cf_type_of_request`). **B14-unpaid-invoice-spm-follow-up does not change group** — keep SPM `159000485013` and set `cf_type_of_request: Follow up on an Unpaid Invoice` + `cf_sp`.
...
| B14-unpaid-invoice-spm-follow-up | SPM team (stay in SPM group) | Unpaid/statement body intent needs SP ID + `Follow up on an Unpaid Invoice` field for SPM follow-up — not invoice intake close (#56213) |
...
### B14-unpaid-invoice-spm-follow-up — SPM unpaid / statement follow-up (added 2026-07-27, v1.18)

**Trigger:** Email **body** (primary) plus subject/conversation shows the requester is chasing **unpaid / past-due / overdue / outstanding** invoices, payment timing, aging, or a `statement of account` / multi-invoice AR list. Attachment presence (xlsx/pdf statement) does **not** convert this to bucket 4. Classify from body intent — e.g. "please let us know when we will be receiving this week", "overdue since May", "need more overdue invoices paid" (audit ticket **#56213**).

**Not B14 when:**
- Explicit manual-entry / Delinquent SC Invoice intake ask → **B10**
- Incorrect totals / billing mismatch needing VINT → **B11**
- Corp AP `SR Requiring Updates` / deposit verification → **B13**
- Single-SR ask with a clean Gateway payment-status answer and no multi-invoice unpaid chase → narrow **Bucket 3** only

**Required action (never auto-close, never leave SPM):**

1. **Read the full body and conversation.** Extract company name, requester email, SR#s, invoice#s, and dollar totals from the text (and statement table if present). Do not classify from subject or filename alone.
2. **Identify the SP** before field updates:
   - Prefer **`sp-inbound-vetting`** (Skills MCP `get_skill("sp-inbound-vetting")`) for company/contact/email/SR identity, **or**
   - Cross-reference **Gateway**, **VixxoLink**, and/or **VixxoNow** with the extracted identifiers.
   - Record SP number and legal/DBA name for `cf_sp`. If unresolved, set `cf_sp` to best-known company string from the body and note `sp-identity-unverified` in the private note + tags.
3. `conversations_manage create_note` (private) — summarize: unpaid/statement ask; SP identity + source (vetting / Gateway / VixxoLink / VixxoNow); key SRs/invoices/amounts from the body; explicit line: `Bucket B14 — unpaid invoice follow-up. Stay in SPM for team follow-up with the SP. Do NOT forward to invoices@, do NOT set manual-entry type, do NOT close (audit #56213).`
4. `tickets_manage update` — use the **B14 SPM unpaid follow-up body** below. **`cf_type_of_request` must be `Follow up on an Unpaid Invoice`** (not `Submit an Invoice for manual entry`). **`group_id` must remain SPM `159000485013`.**
5. **Do NOT** `/forward` to `invoices@vixxo.com`, **do NOT** move to VINT or Corp AP, **do NOT** close, **do NOT** promise a payment date in any SP-facing reply.
6. Optional brief receipt reply only after operator approval (no payment commitment).

**B14 SPM unpaid follow-up body:**
...
```json
{
  ...
  "type": "Invoice Support",
  "group_id": 159000485013,
  "tags": ["unpaid-invoice-follow-up", "spm-unpaid-follow-up"],
  "custom_fields": {
    "cf_sp": "<SP # and/or name from vetting / Gateway / VixxoLink / VixxoNow>",
    "cf_sr": "<primary SR# from body, or N/A if multi-SR statement>",
    "cf_amount": "<stated past-due total or N/A>",
    "cf_sr_required": "Yes",
    "cf_customer": "<whitelist value or Other>",
    "cf_type_of_request": "Follow up on an Unpaid Invoice"
  }
}
```

Merge `qsiap-source` into tags when the QSI AP overlay matches. If SP identity was not confirmed, also merge `sp-identity-unverified`.
...
6. **B14-unpaid-invoice-spm-follow-up:** always escalate — body-context unpaid/statement chase; identify SP via inbound vetting / Gateway / VixxoLink / VixxoNow; set `cf_type_of_request: Follow up on an Unpaid Invoice`; **stay SPM**; leave Open. Never bucket-4 close or manual-entry type (audit #56213).
7. **Ambiguous classification:** more than one rule scores strongly, subject and body suggest different buckets, or the content is genuinely unclear.
8. **Bucket 6 that needs a human next action:** mystery payment reconciliation, credit/adjustment requests post-payment, SP asking questions Vixxo AP can't answer from Gateway alone.
9. **No-attachment SP/provider update chase:** requester asks why payment, invoice approval, NTE approval, rejection correction, or SR movement has not happened and no attachment is present. Route to AP Team, tag `ap-team-routed` + `sp-update-request`, leave Open. Do not close as Bucket 3 or Bucket 6 even when Gateway reveals the likely root cause.
10. **Bucket 7 no-SR:** no SR number anywhere in subject/body/attachments, can't auto-route to Account Team, needs a human to open the attachment and identify the customer.
11. **Phishing soft-indicators only (no strong indicator):** 2+ soft indicators but nothing conclusive — operator reviews before spam/close.
12. **Any ticket where the customer-facing reply would contain a dollar amount > $10,000 or would be the first touch on an SP Vixxo has flagged for quality issues** (e.g., current WERCS pattern). Auto-generated replies on high-$ or flagged-SP tickets are reputational risk.
13. **Tickets older than 14 days:** the aging-bias rule — very old tickets often have context the skill can't see. Surface for review.
14. **X-bec-banking cluster:** 3+ BEC/banking-change attempts targeting the same vendor name or bank account within a rolling 7-day window. Stop auto-firing and escalate to AP Risk + verified vendor contact before closing any more of that cluster. (Individual BEC attempts still auto-fire per the X-bec-banking pattern above; the escalation is for the *cluster*.)
...
| B14-unpaid-invoice-spm-follow-up | `Follow up on an Unpaid Invoice` (unpaid/statement chase — **required**; stay SPM Open; never manual-entry) |
...
- **"Statement" / unpaid / past-due body intent is B14 (stay SPM), not bucket 4 and not a default Bucket 3 close.** Read the email body: if the SP is chasing unpaid invoices or sending a statement of account for payment follow-up, identify the SP, set `cf_type_of_request: Follow up on an Unpaid Invoice`, keep the SPM group, leave Open. Ticket **#56213** was wrongly closed with `invoices-forward` + manual-entry type.
- **Narrow Bucket 3 remains** only for single-SR payment-status answers with a clean Gateway result and no multi-invoice unpaid chase.
...
- **Operator audit (2026-07-27)** — **#56213** Facility Plus statement / past-due unpaid invoices: body asked for payment timing on unpaid invoices; skill wrongly applied invoice-intake style fields (`Submit an Invoice for manual entry`, `invoices-forward`) and closed. Correct path is **B14**: body-context classification, SP identity via inbound vetting / Gateway / VixxoLink / VixxoNow, `cf_type_of_request: Follow up on an Unpaid Invoice`, stay SPM, leave Open for SPM team follow-up.

---

## Version history

- **v1.18 (2026-07-27)** — **B14-unpaid-invoice-spm-follow-up.** Unpaid / past-due / statement-of-account tickets are classified from **email body context**, not attachment presence or subject alone. Required path: identify SP via `sp-inbound-vetting` and/or Gateway / VixxoLink / VixxoNow → set `cf_type_of_request: Follow up on an Unpaid Invoice` + `cf_sp` → **remain in SPM** (`group_id:159000485013`) → leave **Open** for SPM team SP follow-up. Explicitly forbids bucket-4 `/forward`+close, manual-entry type, and group moves off SPM for this ask class. Narrow Bucket 3 retained only for single-SR Gateway payment-status answers. Audit ticket **#56213**.
