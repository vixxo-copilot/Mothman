...
| Account Team / VixxoLink blocker | Portal cannot submit because travel/rate/NTE/product fields are missing, PO is not generating, only `Build Quote` is available, description/dropdown is blank, old/new portal confusion, login/access issue, provider setup, customer/SAP/PO, NTE, rate, or portal-visibility blocker. **Also:** reply to APHelp past-due campaigns where the requester message (before quoted history) asks for VixxoLink login/account setup — even with past-due language in quoted text or a portal screenshot attachment (audit **#70128**). | **B14-vixxolink-portal-vint.** Evaluate requester message before quoted thread. **`group_id:159000486559` (VINT)**, **`type:VixxoLink support`**, tags `vixxolink-portal-support` + `vint-routed`. **Leave Open.** **Do NOT** forward to `invoices@vixxo.com`. Clear-hold must not upgrade to bucket-4 forward solely because an attachment exists. |
