...
def is_vixxolink_support_ask(text: str) -> bool:
    ...
    if _has(
        r"\b("
        r"log ?out|logout|check(?:ing)? out|clock(?:ing)? out|"
        r"log ?in|login"
        r")\b",
        combined,
    ) and _has(r"\b(work order|service request|sr\b)\b", combined):
        return True
    if _has(r"\b(vixxolink|portal)\b", combined) and _has(
        r"\b("
        r"support|log ?in|login|log ?out|logout|password|access|account setup|set up|setup|"
        r"cannot submit|can't submit|dispatch board|accept work|app\b|"
        r"check(?:ing)? out|clock(?:ing)? out"
        r")\b",
        combined,
    ):
        return True
